...
std::string Util::StrFromOct(const std::string& p_Str)
{
  std::string rv;
...
{
  // Install crash handlers via sigaction with SA_SIGINFO so the handler receives
  // the ucontext_t. It is unused by the execinfo path (glibc/macOS, which walk
  // the stack via backtrace()), but the musl path needs it to seed a frame-
  // pointer walk from the interrupted register state -- see GetCallstack().
  struct sigaction crashAction;
  memset(&crashAction, 0, sizeof(crashAction));
  crashAction.sa_sigaction = SignalCrashHandler;
  ...
  {
    sigaction(crashingSignal.first, &crashAction, nullptr);
  }
...
#if !defined(HAVE_EXECINFO_H) && defined(NMAIL_BUILD_MUSL)
// Stack backtrace for platforms without execinfo/backtrace() (musl).
//
// Deliberately scoped to musl (NMAIL_BUILD_MUSL, set by CMake from the target
...
//      ordered, but only where a frame pointer is maintained -- nmail's own code
//      is built -fno-omit-frame-pointer (CMakeLists.txt), the static deps
//      (OpenSSL, libetpan, ...) and musl itself are not. When an async signal
//      lands in that frame-pointer-less code (or in a thread's entry frame, fp=0),
//      the chain breaks at the first hop and the walk yields only the faulting pc
//      -- this is why an x86_64 crash during setup logged a single address.
...
//      pointers, so scanning the thread's stack for words that point into nmail's
...
//      walk succeeds.
//
// Both tiers filter return addresses to nmail's own text range
...
// `addr2line -e nmail.debug <addr>`, no ASLR rebasing.
...
static int FramePointerBacktrace(void** p_Callstack, int p_MaxSize, void* p_Context)
{
  ...
  // own mapping so reads cannot fault, for words pointing into nmail's text. If
...
#endif // !HAVE_EXECINFO_H && NMAIL_BUILD_MUSL

int Util::GetCallstack(void** p_Callstack, int p_MaxSize, void* p_Context)
{
...
void Util::SignalCrashHandler(int p_Signal, siginfo_t* p_SigInfo, void* p_Context)
{
  UNUSED(p_SigInfo);
  char logMsg[64];
...
void Util::CopyFile(const std::string& p_SrcPath, const std::string& p_DstPath)
{
...
void Util::CopyFiles(const std::string& p_SrcDir, const std::string& p_DstDir)
{
  ...
  {
    CopyFile(p_SrcDir + "/" + file, p_DstDir + "/" + file);
  }
}
